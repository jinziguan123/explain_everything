...
Phase 5: review_insights 不再接 gains 参数 —— 从 state.last_gains 读。
这里 fixture 在构造 state 时直接把 last_gains 灌进去（模拟 score_all 跑过之后）。
"""
...
class TestReviewInsights:
    def test_keep_all(self) -> None:
        ...
        with patch("rich.prompt.Prompt.ask", side_effect=["k", "k", "k"]):
            review_insights(state, console=Console(file=None))
        # 3 abstract 全留
