"""Phase 19 Wave 3 Task 17 重写: `enter_repl_async` 用 textual TUI App.
...
新 enter_repl_async 5 行:
1. make_llm_client / make_light_llm_client (LLM 未配 → None, slash 仍可用).
2. init_lexicon_backend (PG 可达 → PG; 断 → 本机 JSON fallback).
3. 建 EphemeralChatSession (启始 chat var).
4. 建 ExplainChatApp(llm, light_llm, ephemeral_chat).
...
设计: docs/plans/2026-05-27-phase-19-tui-design.md §3.3.
"""
...
async def enter_repl_async() -> None:
    ...
    无参数 (跟 cli.main callback `asyncio.run(enter_repl_async())` 对齐).
    内部自己 make_llm_client (env 未配 → None, slash 仍可 work).

    流程:
    1. make_llm_client / make_light_llm_client (KeyError → None).
    2. init_lexicon_backend (best-effort).
    3. EphemeralChatSession(storage, llm) 启始 chat.
    4. ExplainChatApp(llm, light_llm, ephemeral_chat).run_async() 启 TUI.
    ...
    # 2. Lexicon backend init (PG / JSON fallback) — best-effort, 不抛.
