"""Phase 9 Wave F.1: 6 default slash commands (local intercept, bypass LLM).
...
- handler: async (chat, args: list[str]) -> list[ChatEvent]
  返 list[ChatEvent] (multiple events 可能, 但当前 6 个 handler 都 single).
...
# Registry — 6 default slash commands.
...
def _command_by_name(name: str) -> SlashCommand | None:
    """Linear lookup OK — 6 commands, no hash table needed."""
    for cmd in DEFAULT_COMMANDS:
        if cmd.name == name:
            return cmd
    return None
